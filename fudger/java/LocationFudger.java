...
public class LocationFudger {
    ...
    public LocationFudger(float accuracyM) {
        this(accuracyM, Clock.systemDefaultZone(), new SecureRandom());
    }
    ...
    public double roundLon(double longitude, double latitude) {
        double lonGranularity = metersToDegreesLongitude(mAccuracyM, latitude);
        return wrapLongitude(Math.round(longitude / lonGranularity) * lonGranularity);
    }
    public double roundLat(double latitude) {
        double latGranularity = metersToDegreesLatitude(mAccuracyM);
        return wrapLatitude(Math.round(latitude / latGranularity) * latGranularity);
    }

    public double[] getOffset(Location fine) {
